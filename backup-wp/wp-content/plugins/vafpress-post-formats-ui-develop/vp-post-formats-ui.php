...
function vp_pfui_add_meta_boxes($post_type) {
	if (post_type_supports($post_type, 'post-formats') && current_theme_supports('post-formats')) {
		...
		/*add_action('add_meta_boxes', 'vp_pfui_post_admin_setup');*/
		/*add_action('edit_form_after_title', 'vp_pfui_post_admin_setup');*/
		
		add_meta_box( 
			'vp-post-format-meta',
			__( 'Post Format Content <small style="display:block;color:#777;font-weight:400;font-size:13px;"><strong style="color:red;">Important:</strong> Make sure to also select the post format you want to use via the post format dropdown selector in the <strong>Status and Visibility</strong> box</small>' ),
