 ...
 */
if(!class_exists('solopine_latest_news_widget')) {
...
class solopine_latest_news_widget extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'solopine_latest_news_widget', 'description' => esc_html__('A widget that displays your latest posts from all categories or a certain', 'florence-core') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'solopine_latest_news_widget' );

		/* Create the widget. */
		parent::__construct( 'solopine_latest_news_widget', esc_html__('Florence: Latest Posts', 'florence-core'), $widget_ops, $control_ops );
	}
			...
			<?php  while ($loop->have_posts()) : $loop->the_post(); ?>
				...
				<div class="side-pop overlay">
					...
					<div class="side-pop-img">
						...
						<?php else : ?>
						<a href="<?php the_permalink() ?>"><img src="<?php echo get_template_directory_uri(); ?>/img/default-misc.png" alt="<?php esc_attr_e('Default', 'florence-core'); ?>" /></a>
						<?php endif; ?>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Latest Posts', 'florence-core'), 'number' => 3, 'categories' => '', 'layout' => '', 'date' => false, 'counter' => false, 'postids' => '');
		...
		<!-- Layout -->
		<p>
		<label for="<?php echo esc_attr($this->get_field_id('layout')); ?>"><?php esc_html_e( 'Choose Layout', 'florence-core' ); ?>:</label> 
		<select id="<?php echo esc_attr($this->get_field_id('layout')); ?>" name="<?php echo esc_attr($this->get_field_name('layout')); ?>" class="widefat categories" style="width:100%;">
			<option value='small_thumb' <?php if ('small_thumb' == $instance['layout']) echo 'selected="selected"'; ?>><?php esc_html_e( 'Small Thumbnail', 'florence-core' ); ?></option>
			<option value='big_thumb' <?php if ('big_thumb' == $instance['layout']) echo 'selected="selected"'; ?>><?php esc_html_e( 'Big Thumbnail', 'florence-core' ); ?></option>
			<option value='big_thumb_overlay' <?php if ('big_thumb_overlay' == $instance['layout']) echo 'selected="selected"'; ?>><?php esc_html_e( 'Big Thumbnail w/ Overlay', 'florence-core' ); ?></option>
		</select>
		</p>
		
		<!-- Category -->
		<p>
		<label for="<?php echo esc_attr($this->get_field_id('categories')); ?>"><?php esc_html_e( 'Filter by Category', 'florence-core' ); ?>:</label> 
		...
		<!-- Post IDs -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'postids' )); ?>"><?php esc_html_e('Post IDs (separate with comma):', 'florence-core'); ?></label>
			...
			<small><?php esc_html_e( 'Display specific posts using post IDs. This option will override the category option above.', 'florence-core' ); ?></small>
		</p>
		
		<!-- Number of posts -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"><?php esc_html_e('Number of posts to show:', 'florence-core'); ?></label>
		...
		<!-- Numbering -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'counter' )); ?>"><?php esc_html_e( 'Add Numbering', 'florence-core' ); ?>:</label>
		...
		<!-- Date -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'date' )); ?>"><?php esc_html_e( 'Hide Date', 'florence-core' ); ?>:</label>
