...
class solopine_about_widget extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'solopine_about_widget', 'description' => esc_html__('A widget that displays an About widget', 'florence-core') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'solopine_about_widget' );

		/* Create the widget. */
		parent::__construct( 'solopine_about_widget', esc_html__('Florence: About Me', 'florence-core'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<!-- image url -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'image' )); ?>"><?php esc_html_e( 'Image URL', 'florence-core' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Enter the image URL you want to use. You can upload your image via Media > Add New', 'florence-core' ); ?></small>
		</p>

		<!-- description -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'description' )); ?>"><?php esc_html_e( 'About me text', 'florence-core' ); ?>:</label>
