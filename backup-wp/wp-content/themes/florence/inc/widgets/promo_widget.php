...
class florence_promo_widget extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'florence_promo_widget', 'description' => esc_html__('Display a promo box with an image and link', 'florence') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'florence_promo_widget' );

		/* Create the widget. */
		parent::__construct( 'florence_promo_widget', esc_html__('Florence: Promo Box', 'florence'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<!-- Background URL -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'image_url' )); ?>"><?php esc_html_e( 'Background Image URL', 'florence' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Enter the background image URL you want to use. You can upload your background image via Media > Add New', 'florence' ); ?></small>
		</p>
		
		<!-- Image Text -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'heading' )); ?>"><?php esc_html_e( 'Promo Text', 'florence' ); ?>:</label>
		...
		<!-- Link -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'image_link' )); ?>"><?php esc_html_e( 'Promo Link', 'florence' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Enter a link you want the promo box to go to.', 'florence' ); ?></small>
		</p>
		
		<!-- Height -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'height' )); ?>"><?php esc_html_e( 'Promo Box Height', 'florence' ); ?>:</label><br>
		...
		<!-- Margin -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'margin' )); ?>"><?php esc_html_e( 'Promo Box Margin Bottom', 'florence' ); ?>:</label><br>
		...
		<!-- New tab -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'newtab' )); ?>"><?php esc_html_e( 'Open link in a new tab?', 'florence' ); ?></label>
