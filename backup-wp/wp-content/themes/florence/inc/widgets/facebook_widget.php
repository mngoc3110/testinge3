...
class solopine_facebook_widget extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'solopine_facebook_widget', 'description' => esc_html__('A widget that displays a Facebook Like Box', 'florence') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'solopine_facebook_widget' );

		/* Create the widget. */
		parent::__construct( 'solopine_facebook_widget', esc_html__('Florence: Facebook Like Box', 'florence'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<!-- Page url -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'page_url' )); ?>"><?php esc_html_e( 'Facebook Page URL', 'florence' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Example. https://www.facebook.com/envato', 'florence' ); ?></small>
		</p>

		<!-- Faces -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'faces' )); ?>"><?php esc_html_e( 'Show Faces', 'florence' ); ?>:</label>
		...
		<!-- Stream -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'stream' )); ?>"><?php esc_html_e( 'Show Stream', 'florence' ); ?>:</label>
		...
		<!-- Cover -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'cover' )); ?>"><?php esc_html_e( 'Show Page Cover Image', 'florence' ); ?>:</label>
